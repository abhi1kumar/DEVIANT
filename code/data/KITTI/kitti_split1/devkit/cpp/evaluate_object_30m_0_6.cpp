/*
    Sample Use:
    ./splits_prep/kitti_split1/devkit/cpp/evaluate_object './output/kitti_3d_v2_step/results/results_test' 'data/kitti_split1/validation/label_2'

    Version 1 2020-07-23 Abhinav Kumar Plots save commented and gt_dir added. Number of images is automatically read
    from the gt_dir
*/
...
// the threshold for depth. boxes with depth <= DEPTH_THRESH are considered valid
const double DEPTH_THRESH      = 30;
...
/*=======================================================================
FUNCTIONS TO GET BASENAME FROM FILENAME
=======================================================================*/
string get_base_name(const string& path)
{
  size_t found;
  found= path.find_last_of("/\\");
  string foldername = path.substr(0,found);
  string basename   = path.substr(found+1);
...
void cleanData(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det, vector<int32_t> &ignored_gt, vector<tGroundtruth> &dc, vector<int32_t> &ignored_det, int32_t &n_gt, DIFFICULTY difficulty){
  ...
  for(int32_t i=0;i<gt.size(); i++){
    ...
    bool ignore = false;
    if(gt[i].occlusion>MAX_OCCLUSION[difficulty] || gt[i].truncation>MAX_TRUNCATION[difficulty] || height<=MIN_HEIGHT[difficulty] || gt[i].t3 > DEPTH_THRESH || gt[i].t3 <= DEPTH_THRESH-15)
      ignore = true;
...
bool eval(string result_sha, string gt_dir, Mail* mail){
  ...
  //Get all files in the detection directory
  glob_t glob_result;
  glob((result_sha + "/data/*.txt").c_str(), GLOB_TILDE, NULL, &glob_result);
  int32_t N_TESTIMAGES = glob_result.gl_pathc;
  //std::cout<<"Number of images" << N_TESTIMAGES << std::endl;
  ...
  for (int32_t i=0; i<N_TESTIMAGES; i++) {
    ...
    // Reference https://stackoverflow.com/a/7352131
    const char *file_name_const = (get_base_name(glob_result.gl_pathv[i])).c_str();
    std::strncpy(file_name, file_name_const, 255);
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc!=3 && argc!=5) {
    cout << "Usage: ./eval_detection result_sha gt_dir [user_sha email]" << endl;
    return 1;
  }
  ...
  // run evaluation
  if (eval(result_sha, gt_dir, mail)) {
