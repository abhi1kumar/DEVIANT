  ...
  for ind_f in range(3, -1, -1):
    ...
    for j in range(4):
      cv2.line(image, (corners[f[j], 0], corners[f[j], 1]),
               (corners[f[(j+1)%4], 0], corners[f[(j+1)%4], 1]), c, 4, lineType=cv2.LINE_AA)
    # if ind_f == 0:
...
def draw_box_2d(image, corners, c=green_color):
  ...
  cv2.line(image, (left, bottom), (right, bottom), c, 1, lineType=cv2.LINE_AA)
  return image
...
def plot_points_on_image(projected_points, camera_image, rgba_func,bbox_2d,save_path,
                         point_size=5.0):
